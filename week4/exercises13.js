...
function countProfit(shoppers) {
  ...
  for (var j = 0; j < shoppers.length; j++) {
    for (var k = 0; k < dataarr.length; k++) {
      if (shoppers[j].product === dataarr[k].product) {
        if (dataarr[k].leftOver >= shoppers[j].amount) {
          dataarr[k].shoppers.push(shoppers[j].name)
          dataarr[k].leftOver = dataarr[k].leftOver - shoppers[j].amount
          dataarr[k].totalProfit = dataarr[k].totalProfit + (shoppers[j].amount *listBarang[k][1])
        }
      }
    }
  }
...
console.log(countProfit([{name: 'Windi', product: 'Sepatu Stacattu', amount: 8}, {name: 'Vanessa', product: 'Sepatu Stacattu', amount: 10}, {name: 'Rani', product: 'Sweater Uniklooh', amount: 1},
